                ...
                <form @submit.prevent="save">
                    <div class="form-row">
                        <div class="form-col-12 sm:form-col-6">
                            <div class="mb-5">
                                <label for="name" class="mx-field-title required">{{ $t("label.name") }}</label>
                                <input v-model="props.form.name" v-bind:class="errors.name ? 'invalid' : ''" type="text"
                                    id="name" class="mx-field-control" />
                                <small class="mx-field-alert" v-if="errors.name">{{ errors.name[0] }}</small>
                            </div>

                            <div class="mb-5">
                                <label for="email" class="mx-field-title">{{ $t("label.email") }}</label>
                                <input v-model="props.form.email" v-bind:class="errors.email ? 'invalid' : ''"
                                    type="email" id="email" class="mx-field-control" />
                                <small class="mx-field-alert" v-if="errors.email">{{ errors.email[0] }}</small>
                            </div>

                            <div class="mb-5">
                                <label for="phone" class="mx-field-title">{{ $t("label.phone") }}</label>
                                <input v-model="props.form.phone" v-bind:class="errors.phone ? 'invalid' : ''"
                                    type="text" id="phone" class="mx-field-control" />
                                    ...
                                    <div class="mx-field-radio">
                                        <div class="custom-radio">
                                            <input :value="enums.statusEnum.ACTIVE" v-model="props.form.status"
                                                id="active" type="radio" class="custom-radio-field" />
                        ...
                        <div class="form-col-12 sm:form-col-6">
                            <div class="mb-5">
                                <label class="mx-field-title"
                                    for="latitude">{{ $t("label.latitude") }}/{{ $t("label.longitude") }}</label>
                                <div class="mx-multiple-field">
                                    <input v-model="props.form.latitude" v-bind:class="errors.latitude ? 'invalid' : ''"
                                        type="text" id="latitude" />
                                    <input v-model="props.form.longitude"
                                        v-bind:class="errors.longitude ? 'invalid' : ''" type="text" id="longitude" />
                                    <button @click="add" v-on:click="isMap = true" type="button"
                                        class="fa-solid fa-map-location-dot" data-modal="#branchMap"></button>
                                </div>
                                <small class="mx-field-alert" v-if="errors.latitude">{{ errors.latitude[0] }}</small>
                                <small class="mx-field-alert" v-if="errors.longitude">{{ errors.longitude[0] }}</small>
                            </div>

                            <div class="mb-5">
                                <label for="city" class="mx-field-title required">{{ $t("label.city") }}</label>
                                <input v-model="props.form.city" v-bind:class="errors.city ? 'invalid' : ''" type="text"
                                    id="city" class="mx-field-control" />
                                <small class="mx-field-alert" v-if="errors.city">{{ errors.city[0] }}</small>
                            </div>

                            <div class="mb-5">
                                <label for="state" class="mx-field-title required">{{ $t("label.state") }}</label>
                                <input v-model="props.form.state" v-bind:class="errors.state ? 'invalid' : ''"
                                    type="text" id="state" class="mx-field-control" />
                                <small class="mx-field-alert" v-if="errors.state">{{ errors.state[0] }}</small>
                            </div>

                            <div class="mb-5">
                                <label for="zip_code" class="mx-field-title required">{{ $t("label.zip_code") }}</label>
                                <input v-model="props.form.zip_code" v-bind:class="errors.zip_code ? 'invalid' : ''"
                                    type="text" id="zip_code" class="mx-field-control" />
                                <small class="mx-field-alert" v-if="errors.zip_code">{{ errors.zip_code[0] }}</small>
                            </div>

                            <div class="mb-5">
                                <label for="address" class="mx-field-title required">{{ $t("label.address") }}</label>
                                <textarea v-model="props.form.address" v-bind:class="errors.address ? 'invalid' : ''"
                                    id="address" class="mx-field-control"></textarea>
                                <small class="mx-field-alert" v-if="errors.address">{{ errors.address[0] }}</small>
                            </div>
